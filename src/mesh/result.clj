...
(defn handle-input
  ...
  (cond
    (not-empty (re-matches quit_re raw_query_string))
    (do
      (print " ")
      (when (contains? @global_graph_set @global_graph_name) (persist/write-graph @global_graph data_dir))
      (do (println) (System/exit 0)))
    ...
    (if (not (contains? @global_graph_set (nth (str/split (re-find create_graph_re raw_query_string) #"\s{1,}") 1)))
      (do
        (print " ")
        (create-graphs (subs raw_query_string 6)))
    ...
    (if (contains? @global_graph_set (nth (str/split (re-find delete_graph_re raw_query_string) #"\s{1,}") 1))
      (do
        (print " ")
        (delete-graphs (subs raw_query_string 6)))
      (print " Graph not found."))
    (not-empty (re-matches create_node_re raw_query_string))
    (if (contains? @global_graph_set @global_graph_name)
      (do
        (print " ")
        (spin/spin! #(create-nodes @global_graph_name (subs raw_query_string 6)) {:frames (spin/styles :braille)}))
      (print " Graph not found."))
    (not-empty (re-matches show_node_re raw_query_string))
    (if (contains? @global_graph_set @global_graph_name)
      (do
        (print " ")
        (pprint/print-table (spin/spin! #(show-nodes (subs raw_query_string 4)) {:frames (spin/styles :braille)})))
      (print " Graph not found."))
    (not-empty (re-matches create_edge_re raw_query_string))
    (if (contains? @global_graph_set @global_graph_name)
      (do
        (print " ")
        (spin/spin! #(create-edges @global_graph_name (subs raw_query_string 6)) {:frames (spin/styles :braille)}))
      (print " Graph not found."))
    :else
    (print ""))
  (println))